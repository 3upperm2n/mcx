...
#pragma omp barrier
		...
		if(cfg->respin>1){
			...
			cudaMallocHost((void**)&field, sizeof(float)*dimxyz * gpu[gpuid].maxgate*2);
		}else{
			//field=(float *)calloc(sizeof(float)*dimxyz,gpu[gpuid].maxgate); //the second half will be used to accumulate
			cudaMallocHost((void**)&field, sizeof(float)*dimxyz * gpu[gpuid].maxgate);
		}
...
#pragma omp master
			{
				...
				cudaMallocHost((void**)&Pseed, sizeof(RandType)*RAND_BUF_LEN);
				...
				CUDA_ASSERT(cudaMemcpyAsync(gPseed, Pseed, sizeof(RandType)*RAND_BUF_LEN,  cudaMemcpyHostToDevice, stream1));
...
#pragma omp barrier
		...
		cudaMallocHost((void**)&Ppos, sizeof(float4)*gpu[gpuid].autothread);

		//Pdir=(float4*)malloc(sizeof(float4)*gpu[gpuid].autothread);
		cudaMallocHost((void**)&Pdir, sizeof(float4)*gpu[gpuid].autothread);

		//Plen=(float4*)malloc(sizeof(float4)*gpu[gpuid].autothread);
		cudaMallocHost((void**)&Plen, sizeof(float4)*gpu[gpuid].autothread);

		//Plen0=(float4*)malloc(sizeof(float4)*gpu[gpuid].autothread);
		cudaMallocHost((void**)&Plen0, sizeof(float4)*gpu[gpuid].autothread);

		//energy=(float*)calloc(gpu[gpuid].autothread<<1,sizeof(float));
		cudaMallocHost((void**)&energy, (gpu[gpuid].autothread<<1) * sizeof(float));

		//Pdet=(float*)calloc(cfg->maxdetphoton,sizeof(float)*(detreclen));
		cudaMallocHost((void**)&Pdet, cfg->maxdetphoton * sizeof(float)*(detreclen));


		if(cfg->seed!=SEED_FROM_FILE)
			//Pseed=(uint*)malloc(sizeof(RandType)*gpu[gpuid].autothread*RAND_BUF_LEN);
			cudaMallocHost((void**)&Pseed, sizeof(RandType)*gpu[gpuid].autothread*RAND_BUF_LEN);
		else
			//Pseed=(uint*)malloc(sizeof(RandType)*cfg->nphoton*RAND_BUF_LEN);
			cudaMallocHost((void**)&Pseed, sizeof(RandType)*cfg->nphoton*RAND_BUF_LEN);
...
#pragma omp barrier
		...
		CUDA_ASSERT(cudaMemcpyAsync(gmedia, media, sizeof(uchar) *dimxyz, cudaMemcpyHostToDevice, stream1));

		//CUDA_ASSERT(cudaMemcpy(genergy,energy,sizeof(float) *(gpu[gpuid].autothread<<1), cudaMemcpyHostToDevice));
		CUDA_ASSERT(cudaMemcpyAsync(genergy,energy,sizeof(float) *(gpu[gpuid].autothread<<1), cudaMemcpyHostToDevice, stream1));
		...
		for(timegate=0;timegate<totalgates;timegate+=gpu[gpuid].maxgate){
			...
			for(iter=0;iter<(int)cfg->respin;iter++){
				...
				CUDA_ASSERT(cudaMemcpyAsync(gPpos,  Ppos,  sizeof(float4)*gpu[gpuid].autothread,  cudaMemcpyHostToDevice, stream1));
				CUDA_ASSERT(cudaMemcpyAsync(gPdir,  Pdir,  sizeof(float4)*gpu[gpuid].autothread,  cudaMemcpyHostToDevice, stream1));
				CUDA_ASSERT(cudaMemcpyAsync(gPlen,  Plen,  sizeof(float4)*gpu[gpuid].autothread,  cudaMemcpyHostToDevice, stream1));
				...
				if(cfg->seed!=SEED_FROM_FILE){
					...
					CUDA_ASSERT(cudaMemcpyAsync(gPseed, Pseed, sizeof(RandType)*gpu[gpuid].autothread*RAND_BUF_LEN,  cudaMemcpyHostToDevice, stream1));
				}
				...
				switch(cfg->srctype) {
					case(MCX_SRC_PENCIL): mcx_main_loop<MCX_SRC_PENCIL> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_ISOTROPIC): mcx_main_loop<MCX_SRC_ISOTROPIC> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_CONE): mcx_main_loop<MCX_SRC_CONE> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_GAUSSIAN): mcx_main_loop<MCX_SRC_GAUSSIAN> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_PLANAR): mcx_main_loop<MCX_SRC_PLANAR> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_PATTERN): mcx_main_loop<MCX_SRC_PATTERN> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_FOURIER): mcx_main_loop<MCX_SRC_FOURIER> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_ARCSINE): mcx_main_loop<MCX_SRC_ARCSINE> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_DISK): mcx_main_loop<MCX_SRC_DISK> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_FOURIERX): mcx_main_loop<MCX_SRC_FOURIERX> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_FOURIERX2D): mcx_main_loop<MCX_SRC_FOURIERX2D> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_ZGAUSSIAN): mcx_main_loop<MCX_SRC_ZGAUSSIAN> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_LINE): mcx_main_loop<MCX_SRC_LINE> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
					case(MCX_SRC_SLIT): mcx_main_loop<MCX_SRC_SLIT> <<<mcgrid,mcblock,sharedbuf, stream1>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,gseeddata,gdebugdata,gprogress); break;
				}
...
#pragma omp master
				...
				CUDA_ASSERT(cudaMemcpyAsync(&detected, gdetected,sizeof(uint),cudaMemcpyDeviceToHost, stream1));
				...
				CUDA_ASSERT(cudaMemcpyAsync(Plen0,  gPlen,  sizeof(float4)*gpu[gpuid].autothread, cudaMemcpyDeviceToHost, stream1));
...
#pragma omp critical
				...
				if(cfg->issavedet){
					...
					CUDA_ASSERT(cudaMemcpyAsync(Pdet, gPdet,sizeof(float)*cfg->maxdetphoton*(detreclen),cudaMemcpyDeviceToHost, stream1));
					CUDA_ASSERT(cudaGetLastError());
					if(cfg->issaveseed)
